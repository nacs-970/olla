...
**Domain:** Lightweight Python CLI agent (ReAct loop over local Ollama models, XML-tag tool calling)
**Researched:** 2026-06-10
**Confidence:** HIGH (core stack), MEDIUM (tool-calling reliability claims, version pins for fast-moving packages)
...
**Note on Python floor:** if you want to support older Arch installs or a system Python below 3.10, pin `click==8.1.*` instead (8.1.x supports Python 3.7+) and target Python 3.9+. Given this is a personal/local tool on a current Arch system, 3.10+ is a non-issue and recommended.
...
pip install "ollama>=0.6.2" "click>=8.1,<9"

# Optional (only if PROJECT.md constraint is updated to keep rich — see conflict note)
pip install "rich>=13,<14"

# Dev dependencies
pip install "pytest>=8" "pytest-mock>=3.14" "ruff"
```
...
dependencies = [
    "ollama>=0.6.2",
    "click>=8.1,<9",
    # "rich>=13,<14",   # include only if PROJECT.md constraint confirms this
]
...
Use a `src/` layout (`src/olla/__init__.py`, `src/olla/cli.py`, `src/olla/loop.py`, `src/olla/parser.py`, `src/olla/tools.py`) — Hatchling's default wheel-target detection works cleanly with `src/<package>` and it avoids accidental imports of the package from the repo root during testing (a common source of "works on my machine, fails when installed" bugs).
...
**If you want absolute minimal dependency footprint (only `ollama` as third-party dep):**
...
**If you keep `click` + `rich` per PROJECT.md's literal constraint list:**
...
**If targeting models >=7B with native tool-calling later (v2+ exploration):**
...
PROJECT.md's **Constraints** section lists the dependency set as `ollama`, `rich`, `click` only — but the **Out of Scope** section separately lists "Rich colored terminal output" as deferred polish. These two statements are in tension:
...
**Recommendation:** keep `rich` but scope its v1 usage to streaming output + confirm prompts only (functional, not decorative). This satisfies both statements: `rich` is present and earning its place, but no "colored panels/tables/progress bars" polish is built. Roadmap should make this scoping explicit in whichever phase implements the CLI shell, so it doesn't silently expand into a rich-TUI direction.
...
---
*Stack research for: olla — lightweight Python CLI agent for local Ollama models*
*Researched: 2026-06-10*
